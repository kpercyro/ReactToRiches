...
const FoodServices = () => {
  ...
  const callGetAllOutlets = () => {
    setFranchises([]);
    getAllOutlets().then(res => {
      console.log('getAllOutlets returned: ', res);
      setOutlets(res.data);
    });
  };
  const getAllOutlets = async () => {
    const url = '/api/getAllOutlets';
  ...
  const callGetAllFranchises = () => {
    setOutlets([]);
    getAllFranchises().then(res => {
      console.log('getAllFranchises returned: ', res);
      setFranchises(res.data);
    });
  };
  const getAllFranchises = async () => {
    const url = '/api/getAllFranchises';
          ...
          <>
            <Grid item xs={12} margin={1}>
              ...
              <Button
                ...
                edge="start"
                color="primary"
                disabled={franchises && franchises.length > 0}
                onClick={callGetAllFranchises}
                sx={{
                  '&:hover': {backgroundColor: 'lightblue'},
                  height: '55px',
                  width: '200px',
                  ...
                  <Table sx={{minWidth: 650}}>
                    ...
                    <TableBody>
                      {outlets.map((outlet, index) => {
                        return (
                          <StyledTableRow
                            data-testid={`post-${outlet.id}-tablerow`}
                            key={outlet.id}
                            onClick={() =>
                              window.open(
                                `/viewOutlet/?id=${outlet.id}`,
                                '_self',
                              )
                            }
                            sx={{
                          ...
                          >
                            ...
                            <StyledTableCell>{outlet.summary}</StyledTableCell>
                            <StyledTableCell>
                              {outlet.location && outlet.location[0]
                                ? outlet.location[0].name
                                : ''}
                            </StyledTableCell>
                            <StyledTableCell>
                              {outlet.photo ? (
                  ...
                  <Table sx={{minWidth: 650}}>
                    ...
                    <TableBody>
                      {franchises.map((franchise, index) => {
                        return (
                          <StyledTableRow
                            data-testid={`post-${franchise.id}-tablerow`}
                            key={franchise.id}
                            onClick={() =>
                              window.open(
                                `/viewFranchise/?id=${franchise.id}`,
                                '_self',
                              )
                            }
                            sx={{
